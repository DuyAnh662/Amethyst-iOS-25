...
void init_settings() {
    ...
    // iOS: Read settings from environment variables (set by launcher UI),
    // matches the exact options from Android MobileGlues plugin.

    // ANGLE mode: 0=PreferDisabled, 1=PreferEnabled, 2=ForceDisabled, 3=ForceEnabled
    int envAngle = 0;
    GetEnvVarInt("MG_ANGLE", &envAngle, 0);
    switch (envAngle) {
        case 1: global_settings.angle = AngleMode::Enabled; break;
        case 2: global_settings.angle = AngleMode::Disabled; break;
        case 3: global_settings.angle = AngleMode::Enabled; break;
        default: global_settings.angle = AngleMode::Disabled; break;
    }

    // Error ignoring: 0=Auto, 1=DoNotIgnore, 2=IgnoreShaderProgram, 3=IgnoreAll
    int envNoError = 0;
    GetEnvVarInt("MG_NOERROR", &envNoError, 0);
    switch (envNoError) {
        ...
        default: global_settings.ignore_error = IgnoreErrorLevel::None; break;
    }

    // Compute shader: 0=false, 1=true
    int envComputeShader = 0;
    GetEnvVarInt("MG_COMPUTE_SHADER", &envComputeShader, 0);
    global_settings.ext_compute_shader = (envComputeShader > 0);

    // Timer query: 0=enabled (default), 1=disabled (inverted logic: "Disable Recommended timer_query")
    int envTimerQuery = 0;
    GetEnvVarInt("MG_TIMER_QUERY", &envTimerQuery, 0);
    global_settings.ext_timer_query = (envTimerQuery == 0);

    // DSA: 0=false, 1=true
    int envDSA = 0;
    GetEnvVarInt("MG_DSA", &envDSA, 1);
    global_settings.ext_direct_state_access = (envDSA > 0);

    // GLSL cache: -1=disabled, 0=default, >0 = size in MB
    int envGLSLSize = 0;
    GetEnvVarInt("MG_GLSL_CACHE_SIZE", &envGLSLSize, 2);
    if (envGLSLSize < 0) {
        global_settings.max_glsl_cache_size = 0;
    } else if (envGLSLSize == 0) {
        global_settings.max_glsl_cache_size = 2 * 1024 * 1024;
    } else {
        global_settings.max_glsl_cache_size = envGLSLSize * 1024 * 1024;
    }

    // Multidraw: 0=Auto, 1=PreferIndirect, 2=PreferBaseVertex, 3=PreferMultidrawIndirect, 4=ForceDrawElements, 5=PreferCompute
    int envMultidraw = 0;
    GetEnvVarInt("MG_MULTIDRAW", &envMultidraw, 0);
    switch (envMultidraw) {
        case 1: global_settings.multidraw_mode = multidraw_mode_t::PreferIndirect; break;
        case 2: global_settings.multidraw_mode = multidraw_mode_t::PreferBaseVertex; break;
        case 3: global_settings.multidraw_mode = multidraw_mode_t::PreferMultidrawIndirect; break;
        case 4: global_settings.multidraw_mode = multidraw_mode_t::DrawElements; break;
        case 5: global_settings.multidraw_mode = multidraw_mode_t::Compute; break;
        default: global_settings.multidraw_mode = multidraw_mode_t::Auto; break;
    }

    // ANGLE depth clear fix: 0=Disabled, 1=Workaround#1
    int envAngleDepthClear = 0;
    GetEnvVarInt("MG_ANGLE_DEPTH_CLEAR", &envAngleDepthClear, 0);
    global_settings.angle_depth_clear_fix_mode = (envAngleDepthClear > 0) ? AngleDepthClearFixMode::Mode1 : AngleDepthClearFixMode::Disabled;

    // Custom GL version: 0=Disable(default), 32=3.2, 33=3.3, 40=4.0 ... 46=4.6
    int envGLVersion = 0;
    GetEnvVarInt("MG_GL_VERSION", &envGLVersion, 0);
    if (envGLVersion >= 32 && envGLVersion <= 46) {
        global_settings.custom_gl_version = Version(envGLVersion);
    } else {
        global_settings.custom_gl_version = {0, 0, 0};
    }

    // FSR: 0=Disabled, 1=UltraQuality, 2=Quality, 3=Balanced, 4=Performance
    int envFSR = 0;
    GetEnvVarInt("MG_FSR", &envFSR, 0);
    if (envFSR >= 0 && envFSR < static_cast<int>(FSR1_Quality_Preset::MaxValue)) {
        global_settings.fsr1_setting = static_cast<FSR1_Quality_Preset>(envFSR);
    } else {
        global_settings.fsr1_setting = FSR1_Quality_Preset::Disabled;
    }

    // Hide MG env: 0=Disabled, 1=Level1
    int envHideMG = 0;
    GetEnvVarInt("MG_HIDE_ENV", &envHideMG, 0);
    global_settings.hide_mg_env_level = (envHideMG > 0) ? HideMGEnvLevel::Level1 : HideMGEnvLevel::Disabled;

    // Set buffer coherent as flush based on ANGLE mode
    global_settings.buffer_coherent_as_flush = (global_settings.angle == AngleMode::Disabled);

    LOG_V("[MobileGlues iOS] Settings loaded from environment variables")
