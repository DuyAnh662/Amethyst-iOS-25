...
int launchJVM(NSString *username, id launchTarget, int width, int height, int minVersion) {
    ...
    if ([launchTarget isKindOfClass:NSDictionary.class]) {
        ...
        if ([renderer isEqualToString:@ RENDERER_NAME_MOBILEGLUES]) {
            ...
            setenv("MG_DSA", getPrefBool(@"mg.extDirectStateAccess") ? "1" : "0", 1);
    ...
    // Set Vulkan libname for MoltenVK
    ...
    BOOL isJava25 = minVersion >= 25;
    setenv("INTERNAL_JLI_PATH", (isJava8 ? libjlipath8 : libjlipath11).UTF8String, 1);

    // Java 25+ specific flags
    if (isJava25) {
        margv[++margc] = "--enable-preview";
        margv[++margc] = "-XX:+UseZGC";
        margv[++margc] = "-XX:ZCollectionInterval=30";
        margv[++margc] = "-XX:ZFragmentationLimit=10";
    }
